"""Context prefill — read a pack's label so the user does not have to type the form (FR-03).

Two endpoints and no resource. A prefill has no database row, no scan and no history: it is a
question asked of a pack's photographs, answered within a minute, and then gone
...
def _decoded(payload: PrefillCreateIn) -> list[bytes]:
    """Every photograph's bytes, refusing anything malformed or over the ceiling.

    Size is checked **after** decoding, against the real byte count, because base64 length is a
    proxy and the ceiling is about how much the worker will be asked to read. It is a ceiling on
    the *request*, not on one image: what costs time is the total the worker must recognise.
    """
    images: list[bytes] = []
    total = 0
    ...
    for index, entry in enumerate(payload.images):
        try:
            image = base64.b64decode(entry.image_base64, validate=True)
        except (binascii.Error, ValueError) as exc:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail=f"images[{index}].image_base64 is not valid base64",
            ) from exc

        if not image:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail=f"images[{index}].image_base64 decoded to nothing",
            )

        total += len(image)
        if total > settings.PREFILL_MAX_BYTES:
            raise HTTPException(
                status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE,
                detail=(
                    f"the photographs total more than {settings.PREFILL_MAX_BYTES} bytes. "
                    "Downscale before sending — prefill reads words and never measures, so "
                    "thumbnails are enough."
                ),
            )
...
@router.post(
    ...
    summary="Read a pack's photographs to prefill the product context form",
...
def create_prefill(
    ...
    """Accept a pack's photographs and queue them to be read (FR-03).
    ...
    images = _decoded(payload)
    prefill_id = new_prefill_id()
    store = get_object_store()

    keys: list[str] = []
    for index, image in enumerate(images):
        # One key per photograph, ordered, under the same prefill id — so the worker reads them in
        # capture order and deletes exactly what this request wrote.
        key = build_prefill_key(
            org_id=str(principal.org_id),
            prefill_id=f"{prefill_id}-{index}",
            extension=_EXTENSION_FOR[payload.images[index].content_type],
        )

        try:
            # EXIF is stripped even though these objects are deleted minutes from now. It carries
            # the device, the time and often the inspector's coordinates (`storage.strip_exif`),
            # and a short life is not a reason to write it down at all.
            store.put_bytes(key, strip_exif(image), payload.images[index].content_type)
    ...
    enqueue(prefill_id, str(principal.org_id), keys)
...
@router.get(
    ...
    summary="Collect what a pack's photographs suggest for the context form",
    dependencies=[Depends(requires(Permission.SCAN_CREATE))],
)
def get_prefill(
    prefill_id: str,
