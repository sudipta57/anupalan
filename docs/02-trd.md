...
**FR-03 Product context form.** Before or after capture, user supplies: product name, category (searchable list), pack type (`rigid|flexible|glass|can|other`), surface (`printed|embossed`), imported (`yes|no`), and declared net quantity if visible. Fields are pre-filled from OCR and confirmed by the user — the read happens **before** submit, over `POST /v1/prefill`, across **up to three** of the capture's photographs (downscaled), and the screen waits for it behind a loading state so the user confirms a filled form rather than filling one (`docs/decisions.md`, 2026-09-13). The wait always offers a way straight to the blank form, so no network call ever blocks a capture. Name, net quantity and the imported flag may be pre-filled, and the category is matched client-side from the name. When the label prints no common name, the name is asked of the model directly and kept only if every part of it is found in the OCR text; the imported flag may come from a pattern-matched importer line or a country-of-origin line that names a country other than India (`docs/decisions.md`, 2026-09-13); surface never is, because it selects a Rule 9 threshold column and cannot be read from a label's words. Prefill is best-effort: with no network the form is filled by hand, which is the FR-04 path.
