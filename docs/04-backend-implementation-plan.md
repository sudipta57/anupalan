...
| `app/services/vision/` | **Implemented (B5, B6).** `marker.py` + `rectify.py` warp to `PX_PER_MM`; `ocr.py` is the FR-22 interface with stub and PaddleOCR adapters. `orientation.py` re-reads a page photographed on its side and merges the readings — the trigger is the share of taller-than-wide boxes, and an upright capture pays nothing. Metrology (B7) not started. |
...
| `app/services/extraction/` | **Implemented (B9).** Regex, then the LLM over every field with its reading winning on conflict, then `plausibility.py` capping the confidence of a value that could not be its field at all, then human confirmation; every span verified against the real text. |
