...
| `app/services/ingredients/` | **Implemented (B25–B30)**, the online ingredient cross-check of `08-ingredient-crosscheck-plan.md`: `data.py` (the vocabulary and source registry in `ingredients/`, line-level validation, checksums), `normalise.py` and `compare.py` (pure; four outcomes, never a finding), `locate.py` and `split.py`, `identify.py` (pure), `html_text.py`, `fetch.py` (the guarded fetcher — registry hosts and public addresses only, pinned IP with SNI, redirects re-checked, robots.txt), `discover.py` (sitemaps) and `check.py` (orchestrator behind a `CheckStore` port). 234 tests across eight suites, 95% line coverage, and no test opens a socket. **Not wired yet:** no model, migration, task or endpoint (B31–B32). `ingredients/sources-v1.yaml` ships empty pending curation, so today every check is `brand_not_registered`. |
...
| ✅ B8, B29 | `httpx` (promote from dev) | LLM adapters; the ingredient cross-check's guarded fetcher | **Added** as a core dependency (ingredient plan ask 2). HTML is parsed with the stdlib, so nothing else comes with it |
